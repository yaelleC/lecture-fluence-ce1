...
\makeatother
\subtitle{Livret de Lecture --- Fluence CE1 · 16 chapitres d'une même
histoire}
\author{}
\date{}
...
\begin{center}\includegraphics[width=0.75\linewidth]{img/cover_v2.png}\end{center}
...
Ce livret raconte \textbf{une seule histoire}, celle de Camille et de
son chien Mango, en \textbf{16 chapitres}. On les lit dans l'ordre,
comme un petit roman. Les chapitres deviennent peu à peu \textbf{plus
longs et plus riches}, divisés en \textbf{4 sections} de difficulté
croissante :
...
Chaque section commence par une page de présentation qui détaille le
niveau et donne les temps de lecture attendus selon le niveau de classe.

L'histoire est \textbf{originale} et peut être imprimée, copiée ou
modifiée librement.
...
\item
  \textbf{Calculer la vitesse} :
  \texttt{(nombre\ de\ mots\ ÷\ temps\ en\ secondes)\ ×\ 60} = mots par
  minute.
\item
  \textbf{Refaire le même chapitre} 2 ou 3 jours plus tard. La vitesse
  augmente presque toujours. C'est très motivant !
\item
  \textbf{Discuter du chapitre} avec les questions de compréhension.
  Lire, ce n'est pas seulement déchiffrer : c'est aussi comprendre.
\end{enumerate}

\subsubsection{Pour l'enfant}\label{pour-lenfant}

\begin{itemize}
\tightlist
\item
  Tu peux noter ton temps plusieurs fois. Tu verras tes progrès !
\item
  Si tu ne comprends pas un mot, demande à un adulte ou cherche-le dans
  le dictionnaire.
\item
  Et surtout : profite de l'histoire de Camille et de Mango. Tu veux
  savoir la suite ? Alors continue à lire !
\end{itemize}
...
\item
  Pourquoi va-t-elle dans sa cabane quand elle est triste ?
\end{enumerate}
...
Note ici tes records personnels, chapitre après chapitre !

\subsubsection{Section 1 --- Chapitres
courts}\label{section-1-chapitres-courts-1}

{\def\LTcaptype{none} % do not increment counter
\begin{longtable}[]{@{}llll@{}}
\toprule\noalign{}
Chapitre & Mon meilleur temps & Mots / minute & Date \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
1. Moi, c'est Camille & & & \\
2. Ma meilleure amie Suzanne & & & \\
3. Ma cabane secrète & & & \\
...
\bottomrule\noalign{}
\endlastfoot
5. Le premier jour de Mango & & & \\
6. La cabane et le trésor & & & \\
7. On plante un jardin & & & \\
8. La première fois sur la glace & & & \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
9. Les habitants de la forêt & & & \\
10. Mango se perd & & & \\
11. Camille s'entraîne & & & \\
12. Médor, le chien héros & & & \\
\end{longtable}
}
...
\bottomrule\noalign{}
\endlastfoot
13. Le voyage en Écosse & & & \\
14. Les quatre saisons de la forêt & & & \\
15. Le grand spectacle sur glace & & & \\
16. Lettre à Mamie & & & \\
\end{longtable}
}

\textbf{Objectif} : lire de plus en plus vite, et toujours en comprenant
ce qu'on lit. Bravo pour tes efforts, et bravo d'avoir lu toute
l'histoire de Camille et Mango !

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{Note sur l'impression}\label{note-sur-limpression}

Ce livret est écrit en Markdown. Pour l'imprimer en livret PDF :

\begin{itemize}
\tightlist
\item
  \textbf{Méthode simple} : ouvrir le fichier dans une application qui
  prévisualise le Markdown (Typora, Obsidian, VS Code, Marked 2\ldots),
  puis « Imprimer en PDF ».
\item
  \textbf{Méthode plus propre (avec sauts de page)} : utiliser le
  fichier LaTeX et le script \texttt{compile.sh}. Les balises
  \texttt{\textbackslash{}newpage} placeront chaque chapitre sur une
  nouvelle page.
\end{itemize}

\subsubsection{Conseils typographiques pour les enfants en difficulté de
lecture}\label{conseils-typographiques-pour-les-enfants-en-difficultuxe9-de-lecture}

\begin{itemize}
\tightlist
\item
  Police lisible : \textbf{OpenDyslexic}, \textbf{Lexend},
  \textbf{Andika}, ou à défaut Verdana, Comic Sans MS.
\item
